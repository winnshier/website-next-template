...
# 测试环境 Docker 管理脚本
# 使用方法: ./scripts/docker-staging.sh [up|down|restart|logs|status]
...
print_header() {
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo -e "${YELLOW}🐳 ${ENV_NAME}环境 - Docker 管理${NC}"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
}
...
check_env_file() {
    if [ ! -f "$ENV_FILE" ]; then
        ...
        echo ""
        echo "请先创建环境变量文件："
        echo "  cp config/.env.staging.example $ENV_FILE"
...
# 检查SSL证书（仅警告，不阻止）
check_ssl_certs() {
    if [ ! -f "deploy/certs/staging/fullchain.pem" ] || [ ! -f "deploy/certs/staging/privkey.pem" ]; then
        print_warning "未找到SSL证书文件"
        echo ""
        echo "建议配置SSL证书以启用HTTPS："
        echo "  deploy/certs/staging/fullchain.pem"
        echo "  deploy/certs/staging/privkey.pem"
        echo ""
        echo "生成自签名证书（仅用于测试）："
        echo "  openssl req -x509 -nodes -days 365 -newkey rsa:2048 \\"
        echo "    -keyout deploy/certs/staging/privkey.pem \\"
        echo "    -out deploy/certs/staging/fullchain.pem \\"
        echo "    -subj \"/CN=staging.example.com\""
        echo ""
...
docker_up() {
    ...
    echo ""
    print_info "常用命令："
    echo "   查看日志: ./docker-staging.sh logs"
    echo "   停止服务: ./docker-staging.sh down"
    echo "   重启服务: ./docker-staging.sh restart"
    echo "   查看状态: ./docker-staging.sh status"
    echo ""
}
...
docker_rollback() {
    ...
    if [ ! -f ".last-backup-tag" ]; then
        ...
        echo "  ./docker-staging.sh restart"
        exit 1
    fi
...
docker_list_backups() {
    ...
    echo ""
    echo "💡 使用方法："
    echo "   回滚到最近备份: ./docker-staging.sh rollback"
    echo "   手动回滚到指定版本:"
    echo "     docker tag web-tem:backup-YYYYMMDD-HHMMSS web-tem:production"
    echo "     ./docker-staging.sh restart"
    echo ""
...
show_help() {
    print_header
    echo "使用方法: ./docker-staging.sh [命令]"
    echo ""
    echo "可用命令："
    ...
    echo ""
    echo "示例："
    echo "  ./docker-staging.sh up        # 启动服务（自动备份）"
    echo "  ./docker-staging.sh down      # 停止服务"
    echo "  ./docker-staging.sh restart   # 重启服务"
    echo "  ./docker-staging.sh rollback  # 回滚到上一个版本"
    echo "  ./docker-staging.sh backups   # 查看所有备份"
    echo "  ./docker-staging.sh logs      # 查看日志"
    echo "  ./docker-staging.sh status    # 查看状态"
    echo ""
    print_info "测试环境注意事项："
